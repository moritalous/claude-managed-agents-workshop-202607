...
Note that the steps that follow ([3. Create an Agent with the SDK](/en/intermediate/03_sdk/) and the advanced pages) also use these credentials, so **log out only after you have finished everything**. When you come back to continue later, just run `ant auth login` again.
</Aside>
