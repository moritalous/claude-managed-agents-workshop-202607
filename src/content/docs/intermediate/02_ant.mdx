...
1. ブラウザでログインします。
    ...
    `ant auth login --no-browser`（Windowsの場合は`./ant auth login --no-browser`）を実行すると、ブラウザの自動起動は行わず、ターミナルにURLが出力されます。認可するとブラウザにコードが表示されるので、それをターミナルに貼り付けます。
    :::

</Steps>

<Aside type="caution" title="ワークショップが終わったらログアウトを">
`ant auth login`の認証情報はマシンに残り、**Claude Codeで使われることがあります**。詳しくは[こちらのブログ](https://dev.classmethod.jp/articles/claude-max-credit-balance-too-low/)を参照してください。

ワークショップが終わったら、次のコマンドでログアウトしておきましょう。
...
なお、このあとの手順([3. SDKでエージェントを作成](/intermediate/03_sdk/)や上級のページ)もこの認証情報を使います。**ログアウトはすべての手順が終わってから**にしてください。あとから続きをやるときは、もう一度`ant auth login`を実行すれば再開できます。
</Aside>
