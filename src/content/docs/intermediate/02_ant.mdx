...
1. セッションにメッセージを送信し、エージェントの応答を確認します。

    <Tabs syncKey="os">

    <TabItem label="Windows">
    メッセージを送信します(コマンドは1行です)。

    ```shell
    ./ant beta:sessions:events send --session-id $SESSION_ID --event '{type: user.message, content: [{type: text, text: "最新のRAGに関する論文を検索して"}]}'
    ```

    エージェントの処理には1分前後かかります。進行中の様子は、コンソールの「セッション」メニューからこのセッションを開くとリアルタイムに確認できます。

    しばらく待ってから、イベント履歴で回答を確認します。イベントが1行ずつ表示され、`agent.message`の行の`text`が回答です。まだ処理中だった場合は、時間をおいて再実行してください。
    ...
    :::note
    本来は`./ant beta:sessions:events stream --session-id $SESSION_ID`でイベントをリアルタイムに受信できますが、antバージョン1.16.0のWindows版にはストリームが受信できない問題があるため、この手順では履歴の取得で確認しています。
    :::
    </TabItem>

    <TabItem label="macOS">
    メッセージを送信し、続けてイベントをストリームします。ストリームには接続後に発生したイベントだけが流れるため、1行目(送信)と2行目(ストリーム)を**2行まとめてコピーして**実行してください。
    ...
    MCPツールの呼び出し（`agent.mcp_tool_use`）に続いて、`agent.message`イベントで回答が流れてきます。`session.status_idle`が表示されたら完了です。ストリームは`Ctrl+C`で終了できます。
